...
from src.vision_backbone.prismatic.models.backbones.vision.base_vision import ImageTransform, LetterboxPad, VisionBackbone, unpack_tuple
...
class DinoSigLIPViTBackbone(VisionBackbone):
    def __init__(self, vision_backbone_id: str, image_resize_strategy: str, default_image_size: int = 224, reshape=False) -> None:
        ...
        self.dino_featurizer.forward = unpack_tuple(
            partial(self.dino_featurizer.get_intermediate_layers, n={len(self.dino_featurizer.blocks) - 2}, reshape=self.reshape)
        )
        self.siglip_featurizer.forward = unpack_tuple(
            partial(self.siglip_featurizer.get_intermediate_layers, n={len(self.siglip_featurizer.blocks) - 2}, reshape=self.reshape)
        )
    ...
    def forward(self, pixel_values: Dict[str, torch.Tensor]) -> torch.Tensor:
        ...
        return torch.cat([dino_patches[0], siglip_patches[0]], dim=2) if not self.reshape else torch.cat([dino_patches[0], siglip_patches[0]], dim=1)
